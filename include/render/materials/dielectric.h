    ...
    std::optional<SampledDirection> sample(const Vec3& wo, const Vec2& uv, const Vec3& p, const Vec2& sample) const override {
        ...
        */
        ...
        if (sinθ_t > 1.0){
            ...
            return std::optional<SampledDirection>{{eval(ks, uv, p).xyz(), refl}};
        }
        ...
        return std::optional<SampledDirection>{{eval(ks, uv, p).xyz(), w_r}};
    }
