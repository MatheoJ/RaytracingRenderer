...
bool is_stb_image(const std::string &filename)
{
...
bool Image::load(const std::string &filename, bool gamma)
{
    std::string errors_stb, errors_exr;
    ...
    // first try to 
    {
        ...
        bool   isHdr      = stbi_is_hdr(filename.c_str()) || !gamma;
        if (float_data)
        {
            resize(w, h);
            ...
            for (auto y : range(h))
            {
                for (auto x : range(w))
                {
                    auto c = Color4(float_data[4 * (x + y * w) + 0], float_data[4 * (x + y * w) + 1],
                                     float_data[4 * (x + y * w) + 2], float_data[4 * (x + y * w) + 3]);
                    if (!isHdr)
                        c = {to_linear_RGB(c.xyz()), c.w};
                    (*this)(x, y) = c;
                }
            }
    ...
    // next try exrs
    {
        ...
        if (ret == TINYEXR_SUCCESS)
        {
            ...
            {
                for (auto x : range(w))
                    (*this)(x, y) = Color4(float_data[4 * (x + y * w) + 0], float_data[4 * (x + y * w) + 1],
                                            float_data[4 * (x + y * w) + 2], 1.0);
            }
...
void Image::flip_vertically() {
    Image flip_image = *this;
    for (int x = 0; x < sizeX(); ++x) {
        for (int y = 0; y < sizeY(); ++y) {
            flip_image.at(x, y) = (*this).at(x, sizeY() - 1 - y);
        }
    }
    *this = flip_image;
}

bool Image::save(const std::string &filename, float gain)
{
    ...
    else if (extension == "exr")
    {
        ...
        {
            images[0][i] = float((*this)(i)[0]);
            images[1][i] = float((*this)(i)[1]);
            images[2][i] = float((*this)(i)[2]);
        }
        ...
        header.custom_attributes[0].size  = int(strlen(comment));
    ...
    else
    {
        ...
        for (auto y : range(height()))
            ...
            {
                Color3c c{clamp(to_sRGB((*this)(x, y).xyz() * gain), 0.f, 1.f) * 255};
        ...
        if (extension == "png")
            ...
            throw Exception("Could not determine desired file type from extension.");
    }
}